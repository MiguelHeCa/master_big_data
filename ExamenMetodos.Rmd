...
Por la parte de los países, particularmente se muestra que España, Portugal y Alemania oriental forman un *clúster*. Adicionalmente, Holanda, Austria y Alemania Occidental forman parte del clúster 2 (junto con otros paíes), mientras que Rumania, Bulgaria, Yugoslavia pertenecen al tercer conglomerado.
...
A su vez, la sección B \@ref(tab:mdsvar) correspondiente a los países, se puede observar que
...
```{r mdsvar, fig.cap="Plano para variables (A) y países (B) con solución inicial de Torgerson y distancia euclídea al cuadrado", fig.asp=0.6}
...
Por otra parte, \@ref(fig:resi) pone en manifiesto que la relación entre las proximidades transformadas y las distanicas configuradas es fuerte entre ambos conjuntos de valores al formar aproximadamente una línea recta.

```{r shep, fig.cap="Diagrama de Shepard para variables y países"}
par(mfrow = c(1, 2))
...
```{r resi, fig.cap="Gráfico de residuos para variables y países"}
par(mfrow = c(1, 2))
...
Recuerde que el AC puede aplicarse a cualquier matriz de datos positivos, como lo indica @benzecri1973. Trabaje con los valores numéricos como si fueran frecuencias y pondere. NO OLVIDE DEFINIR EL RANGO (25 PAÍSES Y 9 Tipos de PROTEINAS). Utilice el Modelo Principal por columna. Construya los diagramas de dispersión biespacial, puntos de fila y puntos de columna.
...
```{r cos-f, fig.cap="Calidad de representación de variables"}
cos2.f = fviz_ca_row(
  ac_prot,
  col.row = "cos2",
...
```{r cont-f, fig.cap="Contribuciones de variables a las dimensiones"}
cont.f = fviz_ca_row(
  ac_prot,
  col.row = "contrib",
...
```{r cos-c, fig.cap="Calidad de representación de países"}
cos2.c = fviz_ca_col(
  ac_prot,
  col.col = "cos2",
  gradient.cols = c("#f46d43", "#fee08b", "#1a9850"),
...
```{r cont-c, fig.cap="Contribuciones de países a las dimensiones"}
cont.c = fviz_ca_col(
  ac_prot,
  col.col = "contrib",
  gradient.cols = c("#d6604d", "#dedede", "#4393c3"),
