...
El estrés bruto normalizado es muy pequeño (`r round(mds.c$stress, 3)`), lo cual implica un buen ajuste del modelo a los datos.

El plano de representación en dos dimensiones para las variables con el algoritmo `MDS`, con solución inicial de Torgerson, con distancia euclídea al cuadrado calculado y calculado mediante el paquete `smacof` [@smacof2009] se muestra en la figura \@ref(fig:mdsvar). En este gráfico aparecen los cereales y la leche marcando un posición claramente diferente del patrón general. Ambos tipos de carnes parecen comportarse de forma similar, formando clúster con los frutos secos, los huevos, las féculas y el pescado.
...
```{r shpres-c, fig.cap="Relación entre disimilaridades y residuos de variables"}
par(mfrow = c(1, 2))
...
```{r shpres-r, fig.cap="Relación entre disimilaridades y residuos de países"}
par(mfrow = c(1, 2))
